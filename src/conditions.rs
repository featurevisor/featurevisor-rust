...
fn js_equals(left: &AttributeValue, right: &JsonValue) -> bool {
    match (left, right) {
        (AttributeValue::Integer(left), JsonValue::Number(right)) => {
            right.as_i64() == Some(*left) || right.as_f64() == Some(*left as f64)
        }
...
pub(crate) fn parse_condition(value: &JsonValue) -> Result<Condition, String> {
    if let JsonValue::String(value) = value {
        if value == "*" {
            return Ok(Condition::String(value.clone()));
        }
...
pub(crate) fn all_conditions_are_matched(
    ...
    match condition {
        ...
        Condition::Plain(condition) => match condition_is_matched(&condition, context, get_regex) {
            ...
            Err(error) => {
                let mut diagnostic =
                    Diagnostic::new(LogLevel::Warn, "condition_match_error", error.clone());
                diagnostic.original_error = Some(error);
                report(diagnostic);
                false
            }
        },
        Condition::And(value) => value.and.iter().all(|item| {
            all_conditions_are_matched(
        ...
        Condition::Or(value) => value.or.iter().any(|item| {
            all_conditions_are_matched(
                &serde_json::to_value(item).unwrap_or(JsonValue::Null),
                context,
                get_regex,
        ...
        Condition::List(values) => values.iter().all(|item| {
            all_conditions_are_matched(
                &serde_json::to_value(item).unwrap_or(JsonValue::Null),
                context,
                get_regex,
...
pub(crate) fn all_segments_are_matched(
    ...
    if value == JsonValue::String("*".to_string()) {
        return true;
    }
    if let Some(key) = value.as_str() {
        return get_segment(key)
            .map(|segment| {
                all_conditions_are_matched(&segment.conditions, context, get_regex, report)
            })
            .unwrap_or(false);
    }
    if let Some(values) = value.as_array() {
        return values
            .iter()
            .all(|item| all_segments_are_matched(item, context, get_segment, get_regex, report));
    }
    if let Some(object) = value.as_object() {
        if let Some(values) = object.get("and").and_then(JsonValue::as_array) {
            return values.iter().all(|item| {
                all_segments_are_matched(item, context, get_segment, get_regex, report)
            });
        }
        if let Some(values) = object.get("or").and_then(JsonValue::as_array) {
            return values.iter().any(|item| {
                all_segments_are_matched(item, context, get_segment, get_regex, report)
            });
        }
