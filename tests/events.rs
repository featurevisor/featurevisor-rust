...
fn empty_datafile(revision: &str) -> featurevisor::DatafileContent {
    serde_json::from_value(json!({ "schemaVersion": "2", "revision": revision, "segments": {}, "features": {} })).unwrap()
}

#[test]
fn events_are_emitted_in_the_documented_order_and_unsubscribe_is_idempotent() {
    let events = Arc::new(Mutex::new(Vec::new()));
    let observed = Arc::clone(&events);
    let f = create_featurevisor(FeaturevisorOptions::default());
    let unsubscribe = f.on(EventName::DatafileSet, Arc::new(move |_| observed.lock().unwrap().push("datafile")));
...
fn error_diagnostics_emit_error_event() {
    ...
    let f = create_featurevisor(FeaturevisorOptions::default());
    f.on(EventName::Error, Arc::new(move |details| if matches!(details, EventDetails::Error { .. }) { *observed.lock().unwrap() += 1; }));
