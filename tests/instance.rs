use featurevisor::{create_featurevisor, AttributeValue, Context, DatafileInput, FeaturevisorOptions, OverrideOptions};
use serde_json::json;
...
fn sdk() -> featurevisor::Featurevisor {
    ...
    create_featurevisor(FeaturevisorOptions { datafile: Some(DatafileInput::Content(datafile)), ..Default::default() })
}

#[test]
fn evaluates_flag_variation_and_typed_variables() {
    let f = sdk();
    let context: Context = [("userId".to_string(), AttributeValue::from("one"))].into_iter().collect();
    assert!(f.is_enabled("flag", Some(&context)));
    assert!(matches!(f.get_variation("experiment", Some(&context), None).as_deref(), Some("control") | Some("treatment")));
    assert_eq!(f.get_variable_boolean("config", "enabled", Some(&context), None), Some(true));
    assert_eq!(f.get_variable_integer("config", "count", Some(&context), None), Some(3));
    assert_eq!(f.get_variable_string("config", "count", Some(&context), None), None);
}

#[test]
fn defaults_are_presence_based_and_datafiles_merge_or_replace() {
    let f = sdk();
    let value = f.get_variation("missing", None, Some(&OverrideOptions { default_variation_value: Some(String::new()), ..Default::default() }));
...
fn malformed_datafile_preserves_existing_state() {
    ...
    assert!(diagnostics.lock().unwrap().iter().any(|(code, message)| code == "invalid_datafile" && message == "Could not parse datafile"));
}
